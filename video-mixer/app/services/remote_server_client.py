"""Client Remote Server untuk Video Mixer.

Menangani aktivasi, sesi tunggal, heartbeat, grace period, revoke, dan
antrean laporan render tanpa memindahkan proses FFmpeg dari komputer lokal.
"""
...
from version import APP_TYPE, APP_VERSION, REMOTE_SERVER_URL

DEFAULT_SERVER_URL = REMOTE_SERVER_URL
...
def _data_dir() -> Path:
    configured = os.getenv("REMOTE_CLIENT_DATA_DIR", "").strip()
    if configured:
        path = Path(configured)
    elif os.name == "nt":
        root = Path(os.getenv("APPDATA") or Path.home())
        path = root / "VideoMatrixGenerator"
    else:
        root = Path(os.getenv("XDG_CONFIG_HOME") or (Path.home() / ".config"))
        path = root / "video-matrix-generator"
...
def _persistent_host_id() -> str:
    """Return a host identifier persisted in the Docker data volume.

    Docker Desktop on Windows does not expose ``/dev/dri`` or a reliable host
    ``/etc/machine-id``.  A UUID stored in ``/config`` is stable across
    container rebuilds while remaining different on another computer.
    """
    path = _data_dir() / "host-id"
    try:
...
def _machine_identity() -> str:
    host_type = os.getenv("HOST_OS_TYPE", "").strip() or platform.system()
    host_info = os.getenv("HOST_OS_INFO", "").strip() or f"{platform.release()} {platform.machine()}"
    parts: list[str] = [host_type, host_info]
    explicit = os.getenv("VIDEO_MATRIX_HOST_ID", "").strip()
    parts.append(explicit or _persistent_host_id())
    return "|".join(parts)


def fingerprint_hash() -> str:
    raw = f"video-matrix-generator|{_machine_identity()}".encode("utf-8", errors="replace")
    return hashlib.sha256(raw).hexdigest()


def os_type() -> str:
    value = os.getenv("HOST_OS_TYPE", "").strip().lower()
...
def os_info() -> str:
    configured = os.getenv("HOST_OS_INFO", "").strip()
    if configured:
...
def _dpapi_protect(value: str) -> str:
    ...
    if not crypt32.CryptProtectData(
        ctypes.byref(in_blob), "Video Mixer", None, None, None, 0, ctypes.byref(out_blob)
    ):
...
class ClientStore:
    def __init__(self) -> None:
        self.path = _data_dir() / "matrix-remote-server-client.json"
...
class ReportQueue:
    def __init__(self) -> None:
        self.path = _data_dir() / "matrix-report-queue.sqlite3"
        self._lock = threading.RLock()
        conn = self._connect()
        try:
            conn.execute(
...
class RemoteServerClient:
    ...
    def activate(self, code: str) -> dict[str, Any]:
        ...
        if response.status_code != 200 or not body.get("ok"):
            ...
            messages = {
                ...
                "code_app_mismatch": "Kode ini bukan untuk aplikasi Video Mixer.",
    ...
    def _flush_reports(self) -> None:
        ...
        if response.status_code == 200 and body.get("ok"):
            ...
            self._set_state("revoked", False, "Akses Video Mixer dicabut oleh admin.", "revoked")
        else:
    ...
    def _explicit_rejection(self, status_code: int, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if status_code == 403 or status == "revoked":
            self._set_state("revoked", False, "Akses Video Mixer dicabut oleh admin.", status)
    ...
    def _network_failure(self, exc: Exception) -> dict[str, Any]:
        if self._within_grace():
            grace_until = self._grace_until()
            text = "Internet terputus. Video Mixer tetap dapat dipakai dalam grace period."
            if grace_until:
